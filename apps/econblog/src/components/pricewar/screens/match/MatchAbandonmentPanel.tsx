...
export function MatchAbandonmentPanel({
  view,
  reason,
  partialElo,
  coachReport,
  ...
  const iWon = view.outcome.kind === "win" && view.outcome.winner === view.me.slot;
        ...
        <div style={{ marginTop: embedded ? 12 : 18 }}>
          <CoachLessonBlock
            verdict={
              ...
              "A win is a win — but learn the habits that keep you in live matches."
            }
            report={coachReport ?? null}
            fallbackTopic="Don't lose on a no-show"
            fallbackContext="An abandonment carries a rating hit. Learn how to stay connected through a match."
          />
        </div>
      }
    >
        ...
        >
          {iWon ? `${oppFirst} stepped out.` : "Match ended early."}
        </h1>
        <p
          style={{
            fontSize: 14,
